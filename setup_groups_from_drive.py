...
def best_folder_match(group_name: str, folders: list[dict]) -> tuple[dict | None, float]:
    """Return (folder, ratio) for the folder whose slug best matches the group name slug."""
    group_slug = to_slug(group_name)
    best_folder = None
    best_ratio = 0.0
    for folder in folders:
        folder_slug = to_slug(folder["name"])
        ratio = difflib.SequenceMatcher(None, group_slug, folder_slug).ratio()
        if ratio > best_ratio:
            best_ratio = ratio
            best_folder = folder
    return best_folder, best_ratio
...
def main():
    ...
    for group in groups:
        folder, ratio = best_folder_match(group["name"], folders)
        if folder is None or ratio < args.min_match:
